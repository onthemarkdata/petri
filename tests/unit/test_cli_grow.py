"""Unit tests for the ``petri grow`` loop primitives in ``petri.cli``.

These tests exercise the pure ``_grow_loop`` driver in isolation, with
...
from petri.cli import _GrowLoopOutcome, _grow_loop
...
def test_loop_exits_when_all_nodes_terminal() -> None:
    ...
    assert isinstance(outcome, _GrowLoopOutcome)
