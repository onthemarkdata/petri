...
def test_parse_node_id_simple():
    """A four-part node ID parses cleanly."""
    dish, colony_slug, level, seq = parse_node_id("dish-colony-001-002")
    assert dish == "dish"
...
def test_parse_node_id_zero_level_and_seq():
    """Center nodes use level=0, seq=0."""
    dish, colony_slug, level, seq = parse_node_id("mydish-mycol-000-000")
    assert dish == "mydish"
...
def test_parse_node_id_with_hyphens_in_colony_slug():
    ...
    """
    dish, colony_slug, level, seq = parse_node_id(
...
def test_parse_node_id_five_parts():
    """A five-part ID (single-word dish, two-word colony) parses."""
    dish, colony_slug, level, seq = parse_node_id("dish-foo-bar-010-099")
    assert dish == "dish"
...
def test_parse_node_id_raises_on_too_few_parts():
    """Fewer than four hyphen-separated parts is invalid."""
    with pytest.raises(ValueError, match="fewer than 4"):
        parse_node_id("foo-bar")


def test_parse_node_id_raises_on_three_parts():
    """Exactly three parts still fails the 4-part minimum."""
    with pytest.raises(ValueError, match="fewer than 4"):
        parse_node_id("dish-colony-001")


def test_parse_node_id_raises_on_non_integer_level():
    """Non-integer level segment is invalid."""
    with pytest.raises(ValueError, match="level segment"):
        parse_node_id("dish-colony-abc-002")


def test_parse_node_id_raises_on_non_integer_seq():
    """Non-integer seq segment is invalid."""
    with pytest.raises(ValueError, match="seq segment"):
        parse_node_id("dish-colony-001-xyz")


def test_parse_node_id_raises_on_non_string():
    """Non-string input raises ValueError."""
    with pytest.raises(ValueError, match="must be a string"):
        parse_node_id(12345)  # type: ignore[arg-type]
...
def test_node_dir_zero_pads_level_and_seq():
    """level=0, seq=5 produces '000-005'."""
    assert node_dir(Path("/c"), 0, 5) == Path("/c/000-005")


def test_node_dir_pads_three_digits():
    """Single-digit level and seq both get zero-padded to 3 digits."""
    assert node_dir(Path("/colony"), 1, 2) == Path("/colony/001-002")


def test_node_dir_does_not_truncate_large_values():
    """Values requiring more than 3 digits are not truncated."""
    assert node_dir(Path("/c"), 10, 999) == Path("/c/010-999")
    assert node_dir(Path("/c"), 100, 1000) == Path("/c/100-1000")
...
def test_events_and_metadata_compose_with_node_dir():
    """The helpers compose with node_dir to rebuild full paths."""
    node_path = node_dir(Path("/p/petri-dishes/col"), 2, 7)
    assert events_path(node_path) == Path(
        "/p/petri-dishes/col/002-007/events.jsonl"
    )
    assert metadata_path(node_path) == Path(
...
def test_iter_events_files_yields_only_events_jsonl(tmp_path):
    ...
    node_a = colony_a / "000-000"
    node_b1 = colony_b / "001-001"
    node_b2 = colony_b / "001-002"
    for directory in (node_a, node_b1, node_b2):
        directory.mkdir(parents=True)

    (node_a / "events.jsonl").write_text("")
    (node_a / "metadata.json").write_text("{}")
    (node_a / "evidence.md").write_text("")
    (node_b1 / "events.jsonl").write_text("")
    (node_b2 / "events.jsonl").write_text("")
    (node_b2 / "notes.txt").write_text("ignored")
    ...
    expected = sorted(
        [
            node_a / "events.jsonl",
            node_b1 / "events.jsonl",
            node_b2 / "events.jsonl",
        ]
    )
