...
class TestQueueEntry:
    def test_negative_iteration_rejected(self):
        with pytest.raises(ValidationError):
            QueueEntry(cell_id="n1", iteration=-1)

    def test_max_iterations_zero_rejected(self):
        with pytest.raises(ValidationError):
            QueueEntry(cell_id="n1", max_iterations=0)

    def test_negative_cycle_start_rejected(self):
        with pytest.raises(ValidationError):
            QueueEntry(cell_id="n1", cycle_start_iteration=-1)


class TestDecompositionResult:
    def test_composes_cell_and_edge(self):
        cell = Cell(id="n1", colony_id="c1", claim_text="x", level=0)
        edge = Edge(from_cell="n1", to_cell="n2")
        ...
        assert len(decomposition_result.cells) == 1
...
class TestValidateEventData:
    ...
    def test_valid_cell_reopened(self):
        result = validate_event_data(
            "cell_reopened",
    ...
    def test_valid_propagation_triggered(self):
        result = validate_event_data(
            ...
            {"reopened_cell_id": "n1", "flagged_dependents": ["n2"]},
        )
    ...
    def test_valid_decomposition_created(self):
        result = validate_event_data(
            ...
            {"parent_cell_id": "p1", "child_cell_ids": ["c1"]},
        )
...
class TestBuildCellKey:
    def test_basic(self):
        assert (
            build_cell_key("research1", "market", 2, 3)
            == "research1-market-002-003"
        )

    def test_zero_padding(self):
        assert (
            build_cell_key("my-dish", "ai-eval", 0, 0)
            == "my-dish-ai-eval-000-000"
        )

    def test_large_numbers(self):
        assert (
            build_cell_key("d", "c", 100, 999)
            == "d-c-100-999"
        )
...
class TestParseKey:
    ...
    def test_cell_key_with_dish_id(self):
...
class TestParentKey:
    def test_event_to_cell(self):
        assert (
            parent_key("research1-market-002-003-a1b2c3d4")
            == "research1-market-002-003"
        )

    def test_cell_to_colony_level(self):
