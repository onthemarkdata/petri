...
Thanks for your interest! Petri is a colony-based research orchestration framework, and it's in
the middle of an exciting phase: the **v2 migration** — re-platforming onto Pydantic AI,
pydantic-graph, and DBOS while keeping the zero-infrastructure, local-first design.

## Where to start

- **The roadmap:** [docs/v2/MIGRATION_PLAN.md](docs/v2/MIGRATION_PLAN.md) — 7 milestones, each
  with a tracking epic issue and a set of scoped, independently-landable issues.
...
CI (tests on Python 3.11–3.14, ruff lint, package build) runs on every PR.
...
uv run pytest tests/ -q     # 500+ tests, all offline — no API keys, no claude CLI needed
...
1. **Comment on the issue** ("I'd like to take this") — the maintainer will assign it to you.
2. **Check its "Blocked by" relationships first** — if a blocking issue is open, start there
   instead (or pick something unblocked).
3. One issue per PR. The issue's **acceptance criteria checkboxes are the definition of done** —
   your PR should let a reviewer tick every box.
4. Questions welcome — comment on the issue anytime. Unclear scope is a bug in the issue, not
   in you.
...
1. On `dev`: bump `version` in `pyproject.toml`.
2. Open a PR `dev` → `main` titled `Release X.Y.Z`; CI must be green.
3. Merge with **Rebase and merge** (preserves individual commits; `main` requires linear history).
4. `gh release create vX.Y.Z --target main --title "vX.Y.Z" --generate-notes` — the publish
   workflow ships to PyPI via trusted publishing.
5. Resync dev (rebase-merge rewrites SHAs):
   ```bash
...
Be kind, be constructive, assume good faith. Research tooling attracts people from many
backgrounds — welcome them.
